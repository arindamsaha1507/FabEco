...
import ruamel.yaml
import yaml
from plugins.FabEco.FabEco import *
...
def eco_init_SA(config,
                 outdir=".",
                 script="eco",
                 eco_script="eco_game_dynamics.py",
                 sampler_name=None,
                 ** args):
    '''
    ============================================================================
    ...
    ############################################
    eco_SA_config_file = os.path.join(
        get_plugin_path("FabEco"),
        "SA",
        "eco_SA_config.yml"
    )
    eco_SA_campaign_config = load_eco_SA_campaign_config(eco_SA_config_file)
    polynomial_order = eco_SA_campaign_config["polynomial_order"]

    if sampler_name is None:
        sampler_name = eco_SA_campaign_config["sampler_name"]

    ############################
    # Create easyvvuq campaign #
    ############################
    campaign_name = "eco_SA_{}".format(sampler_name)
    campaign_work_dir = os.path.join(
        get_plugin_path("FabEco"),
        "SA",
        "eco_SA_{}".format(sampler_name)
    )

    runs_dir, campaign_dir = init_eco_SA_campaign(
        campaign_name=campaign_name,
        campaign_config=eco_SA_campaign_config,
...
@task
@load_plugin_env_vars("FabEco")
def eco_analyse_SA(config, sampler_name=None, ** args):
    """
    ===========================================================================
    ...
    #####################################################
    output_filename = eco_SA_campaign_config[
    ...
    #######################################
    output_column = eco_SA_campaign_config["decoder_output_column"]
    ...
    with open(eco_SA_config_file, 'r') as ff:
    ...
    yml_results["campaign_info"].update({
        ...
        "polynomial_order": polynomial_order,
        "sampler": S(eco_SA_campaign_config["sampler_name"]),
        "distribution_type": S(eco_SA_campaign_config["distribution_type"]),
        "sparse": S(eco_SA_campaign_config["sparse"]),
        "growth": S(eco_SA_campaign_config["growth"])
    })
    if sampler_name == "SCSampler":
        yml_results["campaign_info"].update({
            "quadrature_rule": S(eco_SA_campaign_config["quadrature_rule"]),
            "midpoint_level1": S(eco_SA_campaign_config["midpoint_level1"]),
            "dimension_adaptive": S(eco_SA_campaign_config["dimension_adaptive"])
        })
    elif sampler_name == "PCESampler":
        yml_results["campaign_info"].update({
            "rule": S(eco_SA_campaign_config["quadrature_rule"]),
        })

    ROUND_NDIGITS = 4
    for param in eco_SA_campaign_config["selected_vary_parameters"]:
...
def load_eco_SA_campaign_config(facs_SA_config_file):
    ...
    #####################################################
    sampler_params_json_PATH = os.path.join(get_plugin_path("FabEco"),
                                            "templates",
                                            "params.json"
                                            )
...
def init_eco_SA_campaign(campaign_name, campaign_config,
    ...
    # Create an encoder #
    #####################
    directory_tree = {"configuration": None}

    encoder = uq.encoders.MultiEncoder(
        uq.encoders.DirectoryBuilder(tree=directory_tree),
        uq.encoders.GenericEncoder(
            template_fname=get_plugin_path("FabEco") +
            "/templates/" + template,
            delimiter="$",
            target_filename="configuration/" + target
        ),
        # CustomEncoder(
